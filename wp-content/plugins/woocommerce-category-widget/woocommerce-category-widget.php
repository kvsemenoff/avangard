...
class woocommerce_category extends WP_Widget {
    ...
    function form($instance) {
        ...
        if ($instance) {
            ...
            if(isset($instance['category_image'])){
        ...
        if (!empty($all_categories)) {
            printf(
                '<select multiple="multiple" name="%s[]" id="%s" class="widefat" size="15" style="margin-bottom:10px">', $this->get_field_name('category_id'), $this->get_field_id('category_id')
            );
            // The Loop
            foreach ($all_categories as $cat) {
                if ($cat->category_parent == 0) {
                    ...
                    if($select == ''){
                        ...
                        if(is_array($select)&&(in_array($category_id, $select))){
                    ...
                    printf(
                        '<option value="%s" class="hot-topic" %s style="margin-bottom:3px;">%s</option>', $category_id, $selected, $cat->name
                    );
                }
            }
        ...
        <p>
            ...
            <?php
            printf(
                '<select name="%s[]" id="%s" >', $this->get_field_name('category_image'), $this->get_field_id('category_image')
            );
            ?>

            <option value="yes" class="hot-topic" <?php echo $category_image[0] == 'yes' ? ' selected="selected"' : ''; ?> style="margin-bottom:3px;">Yes</option>
            <option value="no" class="hot-topic" <?php echo $category_image[0] == 'no' ? ' selected="selected"' : ''; ?> style="margin-bottom:3px;">No</option>
        ...
        <?php
        printf(
            '<select name="%s[]" id="%s" >', $this->get_field_name('show_title'), $this->get_field_id('show_title')
        );
        ?>
            <option value="yes" class="hot-topic" <?php echo $show_title[0] == 'yes' ? ' selected="selected"' : ''; ?> style="margin-bottom:3px;">Yes</option>
            <option value="no" class="hot-topic" <?php echo $show_title[0] == 'no' ? ' selected="selected"' : ''; ?> style="margin-bottom:3px;">No</option>
        <?php
        echo '</select>';
        echo '</p>';
    }
