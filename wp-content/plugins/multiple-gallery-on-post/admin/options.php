...
add_action('admin_init', 'mgop_admin_init');
	...
	function mgop_setting_string() {
		...
		foreach($post_types as $post_name => $post_label){
            if(isset($options[$post_name]['active'])){
                $checked = 'checked';
                $active = '';
            } else {
                $checked = '';
			...
			<div class="mgop-filed">
				<div class="mgop-filed-header">
					<input class="mgop-post-type" data-for="content_<?php echo  $post_name; ?>" id="mgop_post_<?php echo  $post_name; ?>" name="mgop_options[<?php echo  $post_name; ?>][active]" type="checkbox" value="<?php echo  $post_name; ?>" <?php echo $checked; ?> /><label for="mgop_post_<?php echo  $post_name; ?>" class="checkbox-label"><?php echo  $post_label; ?></label>
				</div>
				<div id="content_<?php echo  $post_name; ?>" class="mgop-filed-content<?php echo $active; ?>">
...
function mgop_options_validate($input) {
	...
	if(!isset($options['validated']) && is_array($options)){
