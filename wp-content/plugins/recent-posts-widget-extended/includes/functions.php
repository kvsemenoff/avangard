...
function rpwe_get_recent_posts( $args = array() ) {
	...
	if ( $posts->have_posts() ) :
        ...
        if(isset($args['home_template']) && $args['home_template']){
            // Recent posts wrapper on home page
            ...
            while ( $posts->have_posts() ) : $posts->the_post();
                ...
                $html .= '<li class="post-prew cf">';

                if ( $args['thumb'] ) :

                    // Check if post has post thumbnail.
                    if ( has_post_thumbnail() ) :
                        $html .= '<a class="post-prew_img-wrap" href="' . esc_url( get_permalink() ) . '"  rel="bookmark">';
                        if ( $image ) :
                ...
                $html .= '<a class="post-prew_title" href="' . esc_url( get_permalink() ) . '" title="' . sprintf( esc_attr__( 'Permalink to %s', 'recent-posts-widget-extended' ), the_title_attribute( 'echo=0' ) ) . '" rel="bookmark">' . esc_attr( get_the_title() ) . '</a>';
                ...
                if ( $args['excerpt'] ) :
                    $html .= '<p class="post-prew_content">';
                    $html .= wp_trim_words( apply_filters( 'rpwe_excerpt', get_the_excerpt() ), $args['length'], ' &hellip;' );
                    $html .= '</p>';
                endif;

                if ( $args['all_posts'] ) :
                    $category_id = $args['cat'][0];
                    $cat_link = get_category_link( $category_id );
                    $cat_name = get_the_category_by_ID( $category_id );
                    $cat_name = mb_strtolower($cat_name, 'UTF-8');
                    $html .= '<a href="' . $cat_link . '" class="post-prew_more" title="Показать все '.$cat_name.'">Все ' . $cat_name . '</a>';
                endif;
